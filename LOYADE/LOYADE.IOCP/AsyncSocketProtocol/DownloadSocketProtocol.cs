...
namespace LOYADE.IOCP
{
    public class DownloadSocketProtocol : BaseSocketProtocol
    {
        ...
        public bool DoDir()
        {
            ...
            {
                if (parentDir == "")
                    parentDir = __Global.FileDirectory;
                else
                    parentDir = Path.Combine(__Global.FileDirectory, parentDir);
        ...
        public bool DoFileList()
        {
            ...
            {
                if (dirName == "")
                    dirName = __Global.FileDirectory;
                else
                    dirName = Path.Combine(__Global.FileDirectory, dirName);
        ...
        public bool DoDownload()
        {
                ...
                & m_incomingDataParser.GetValue(ProtocolKey.FileSize, ref fileSize) & m_incomingDataParser.GetValue(ProtocolKey.PacketSize, ref packetSize))
            {
                ...
                __Global.Logger.Info("Start download file: " + fileName);
                ...
                if (File.Exists(fileName))
                {
                    ...
                    else
                    {
                        ...
                        __Global.Logger.Error("Start download file error, file is in use: " + fileName);
                    }
                }
                else
                {
        ...
        public override bool SendCallback()
        {
            ...
            if (m_fileStream != null)
            {
                ...
                else
                {
                    ...
                    {
                        __Global.Logger.Info("End download file: " + m_fileName);
                        m_fileStream.Close();
                        m_fileStream = null;
                        m_fileName = "";
                        m_sendFile = false;

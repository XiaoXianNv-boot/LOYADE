...
namespace LOYADE.MATH
{
    ...
    public class XMatrix
    {
        ...
        //@param nRows--指定的矩阵行数
        //@param nCols--指定的矩阵列数
        public XMatrix(int nRows, int nCols)
        {
            numRows = nRows;
            numColumns = nCols;
            Init(numRows, numColumns);
        ...
        public XMatrix(int nRows, int nCols, double[] value)
        {
            numRows = nRows;
            numColumns = nCols;
            Init(numRows, numColumns);
        ...
        public XMatrix(int nSize, double[] value)
        {
            numRows = nSize;
            numColumns = nSize;
            Init(nSize, nSize);
        ...
        //重载double[]运算符
        //return double[]对象
        public static implicit operator double[](XMatrix m)
        {
            return m.elements;
        }
        //将方阵初始化为单位矩阵
        //@param nSize--方阵行列数
        //@return bool型,初始化是否成功
        ...
        //将矩阵各元素的值转化为字符型,元素之间的分隔符为",",行与行之间有回车换行符
        ...
        //将矩阵指定行中各元素的值转化为字符串
        //@param nRows--指定的矩阵行,nRows=0表示第一行
        //@param sDelim--元素之间的分隔符
        //@return string型,转换得到的字符串
        public string ToStringRow(int nRow, string sDelim)
        {
            ...
            for (int j = 0; j < numColumns; ++j)
            {
                string ss = GetElement(nRow, j).ToString("F8");
                s += ss;
                if (j != numColumns - 1)
                    s += sDelim;
            }
        ...
        public string ToStringCol(int nCol, string sDelim)
        {
            ...
            for (int i = 0; i < numRows; ++i)
            {
                string ss = GetElement(i, nCol).ToString("F8");
                s += ss;
                if (i != numRows - 1)
                    s += sDelim;
            }
        ...
        public XMatrix SetValue(XMatrix other)
        {
            if (other != this)
            {
        ...
        {
            XMatrix matrix = other as XMatrix;
            if (matrix == null)
                return false;
        ...
        public XMatrix Add(XMatrix other)
        {
            //首先检查行列数是否相等
            ...
            //构造结果矩阵
            XMatrix result = new XMatrix(this);   //拷贝构造
        ...
        public XMatrix Subtract(XMatrix other)
        {
            if (numColumns != other.GetNumColumns() || numRows != other.GetNumRows())
                throw new Exception("矩阵的行/列数不匹配。");
        ...
        public XMatrix Multiply(double value)
        {
            //构造目标矩阵
            XMatrix result = new XMatrix(this);    
        ...
        public XMatrix Multiply(XMatrix other)
        {
            //首先检查行列是否符合要求
            if (numColumns != other.GetNumRows())
                throw new Exception("矩阵的行/列数不匹配。");
            //ruct the object we are going to return
            XMatrix result = new XMatrix(numRows, other.GetNumColumns());
            //矩阵乘法,即
            //
        ...
        public bool Multiply(XMatrix AR, XMatrix AI, XMatrix BR, XMatrix BI, XMatrix CR, XMatrix CI)
        {
            //首先检查行列数是否符合要求
            ...
            //构造乘积矩阵实部和虚部矩阵
            XMatrix mtxCR = new XMatrix(AR.GetNumRows(), BR.GetNumColumns());
            XMatrix mtxCI = new XMatrix(AR.GetNumRows(), BR.GetNumColumns());
            //复矩阵相乘
        ...
        public XMatrix Transpose()
        {
            //构造目标矩阵
            XMatrix Trans = new XMatrix(numColumns, numRows);
        ...
        //@return bool型号，求逆是否成功
        public bool InvertGaussJordan(XMatrix mtxImag)
        {
            int i, j, k, l, u, v, w;
            double p, q, s, t, d, b;
        ...
        //@return bool型,求解是否成功
        public bool SpiltLU(XMatrix mtxL, XMatrix mtxU)
        {
            int i, j, k, w, v, ll;
        ...
        //@return bool型,求解是否成功
        public bool SplitQR(XMatrix mtxQ)
        {
            int i, j, k, l, nn, p, jj;
            double u, alpha, w, t;
        ...
        //@param eps---计算精度
        //@return bool型,求解是否成功
        public bool SplitUV(XMatrix mtxU, XMatrix mtxV, double eps)
        {
         ...
         * @return bool型，求解是否成功
         */
        public bool InvertUV(XMatrix mtxAP, XMatrix mtxU, XMatrix mtxV, double eps)
        {
            int i, j, k, l, t, p, q, f;
         ...
         * @return bool型，求解是否成功
         */
        public bool MakeSymTri(XMatrix mtxQ, XMatrix mtxT, double[] dblB, double[] dblC)
        {
            int i, j, k, u;
            double h, f, g, h2;
         ...
         * @return bool型，求解是否成功
         */
        public bool ComputeEvSymTri(double[] dblB, double[] dblC, XMatrix mtxQ, int nMaxIt, double eps)
        {
            int i, j, k, m, it, u, v;
         ...
         * @return bool型，求解是否成功
         */
        public bool ComputeEvJacobi(double[] dblEigenValue, XMatrix mtxEigenVector, int nMaxIt, double eps)
        {
         ...
         * @return bool型，求解是否成功
         */
        public bool ComputeEvJacobi(double[] dblEigenValue, XMatrix mtxEigenVector, double eps)
        {
            int i, j, p, q, u, w, t, s;
